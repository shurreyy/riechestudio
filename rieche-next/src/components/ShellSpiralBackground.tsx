...
interface ShellSpiralBackgroundProps {
  shellType?: 'clam' | 'nautilus' | 'olive' | 'periwinkle' | 'turritella' | 'whelk';
  showControls?: boolean;
  opacity?: number;
}

const ShellSpiralBackground: React.FC<ShellSpiralBackgroundProps> = ({
  shellType = 'nautilus',
  showControls = false,
  opacity = 0.3
}) => {
  ...
  const animate = () => {
    ...
    // Draw preview (only if controls are shown)
    if (showControls) {
  ...
  const setShellParameters = (shellType: 'clam' | 'nautilus' | 'olive' | 'periwinkle' | 'turritella' | 'whelk') => {
      ...
      {showControls && (
        <div style={{
          ...
          {[
            ...
            <button
              key={shell.key}
              onClick={() => setShellParameters(shell.key as 'clam' | 'nautilus' | 'olive' | 'periwinkle' | 'turritella' | 'whelk')}
              style={{
                width: '100px',
                padding: '8px',
...
export default ShellSpiralBackground; 